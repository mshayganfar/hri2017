...
example, \ldots (see Figure XYZ -- for branches!!!). After achieving a new
primitive goal, the robot either informed the human keeping the ground for the
next goal to achieve, or informed and passed the ground to the human to execute
the next task with repoect to the human's goal. In case of the human's turn, the
robot waited for the human to do a task, then the wizard let the robot know
whether the human's goal was achieved or not. Afterwards the robot was making a
decision about which goal to pursue and was informing human accordingly. The
same entire procedure was applied to both conditions.

\subsection{Environment and Tasks}

The environment included the robot, the collaboration board on top of a desk,
and the participant standing in front of the board oposite to the robot in our
lab (see Figure XYZ). The tasks were defined based on the HTN structure shown in
Figure XYZ. For each task either the robot or the participant had to pick one of
the corresponding pegs in their own inventory and place on the right spot
colored and tagged same as the tag on the associated peg.
